...
const HOMEWORK_HISTORY: Homework[] = [
    {
...
function calculateMemberData(): MemberData[] {
    const members = MEMBER_NAMES.map(memberName => {
        const chosenCount = HOMEWORK_HISTORY.reduce(
            (acc, homework) => homework.giver === memberName ? acc + 1 : acc, 0
        );
        return {
            name: memberName,
            chosenCount,
...
function decideHomework(): void {
    ...
    if (memberName) memberName.style.color = 'red';
